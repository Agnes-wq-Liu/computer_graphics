...
public class CharacterFromXML {
	...
	public static GraphNode load( String filename ) {
		try {
			...
			return createScene( null, document.getDocumentElement() ); // we don't check the name of the document element
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 */
	public static double leftOrRight(String lr, double bound) {
		if (lr=="right") return(-1.0*Math.abs(bound));
		else return Math.abs(bound);
	}
	public static GraphNode createJoint( Node dataNode ) {
		...
		if ( type.equals("free") ) {
			...
			String lr = dataNode.getAttributes().getNamedItem("lr").getNodeValue();
			//min-max rotation restrictions for limbs
			if (lr.equals("right arm")) {
				joint.setX(0.0, 0.0);
				joint.setY(-1.0, 1.0);
				joint.setZ(-1.5, -0.7);
			}
			else if (lr.equals("left arm")) {
				joint.setX(0.0, 0.0);
				joint.setY(-1.0, 1.0);
				joint.setZ(0.7, 1.5);
			}
			...
			joint.setMinMax(getTuple3dAttr(dataNode, "rotate"));
			return joint;
