...
public class Parser {
	...
	public static Material createMaterial(Node dataNode) {
		...
		if( refAttr  != null ) {
	    	...
	    	Node reflectAttr = dataNode.getAttributes().getNamedItem("mirror");
	    	if(reflectAttr!=null) { 
	    		if (reflectAttr.getNodeValue().contentEquals("true")) material.mirror = true;
	    	}
	    	Node refracAttr = dataNode.getAttributes().getNamedItem("n");
	    	if(refracAttr!=null) {
	    		material.refracN = Float.parseFloat(refracAttr.getNodeValue());
	    		Node ref2Attr = dataNode.getAttributes().getNamedItem("nt");
	    		if(ref2Attr!=null) {
		    		material.refracNt = Float.parseFloat(ref2Attr.getNodeValue());
	...
	public static Render createRender(Node dataNode) {
		...
		Node jitAttr = dataNode.getAttributes().getNamedItem("jitter");
		if ( jitAttr != null ) {
        	render.jitter= (jitAttr.getNodeValue().equals("true"));
		}
    	NodeList nodeList = dataNode.getChildNodes();
    	for (int i = 0; i < nodeList.getLength(); i++) {
    		Node n = nodeList.item(i);
	...
	public static Quadric createQuadric(Node dataNode) {
		...
		if ( centerAttr != null ) {
            ...
            double y = s.nextDouble();
            double z = s.nextDouble();
            quadric.center = new Point3d(x, y, z);
            s.close();
		}
